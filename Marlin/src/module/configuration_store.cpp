...
typedef struct SettingsDataStruct {
  ...
  //
  uint32_t motor_current_setting[4];                    // M907 X Z E
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
	  ...
	  EEPROM_WRITE(wtvar_enablepoweroff);

	  // save self test enable
	  EEPROM_WRITE(wtvar_enableselftest);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      {
        uint32_t motor_current_setting[4];
        _FIELD_TEST(motor_current_setting);
...
void MarlinSettings::reset() {
  ...
  //
    ...
    #endif
    probe.offset.z = z_backup;      // perron, not reset the z offset
  #endif
  ...
  #if HAS_MOTOR_CURRENT_PWM
    constexpr uint32_t tmp_motor_current_setting[4] = PWM_MOTOR_CURRENT;
    for (uint8_t q = 4; q--;)
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    // perron, 200324
      ...
      SERIAL_ECHOLNPAIR_P(
        ...
        , SP_Y_STR, stepper.motor_current_setting[1]
        , SP_Z_STR, stepper.motor_current_setting[2]
        , SP_E_STR, stepper.motor_current_setting[3]
      );
    #endif
