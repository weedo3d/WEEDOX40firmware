...
void Temperature::max_temp_error(const heater_ind_t heater) {
    ...
    if (heater >= 0)
    {
      ...
      if (dgus.isTesting())
      {
        dgus.NozzleTempError();
      }
      else
      #endif
      {
            _temp_error(heater, PSTR(STR_T_MAXTEMP), MMSG_ERROR_MAXTEMP[wtvar_language]);
    ...
    else
    {
      ...
      #endif
      {
            _temp_error(heater, PSTR(STR_T_MAXTEMP), MMSG_ERROR_BEDTEMP[wtvar_language]);
...
void Temperature::min_temp_error(const heater_ind_t heater) {
    ...
    if (heater >= 0)
    {
      ...
      #endif
      {
            _temp_error(heater, PSTR(STR_T_MINTEMP), MMSG_ERROR_MINTEMP[wtvar_language]);
    ...
    else
    {
      ...
      #endif
      {
            _temp_error(heater, PSTR(STR_T_MINTEMP), MMSG_ERROR_BEDTEMP[wtvar_language]);
