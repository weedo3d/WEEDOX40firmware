  ...
  void Planner::apply_leveling(xyz_pos_t &raw) {
    if (!leveling_active) return;
      ...
      // perron 200814
        ...
        {
            xyz_pos_t raw_copy = raw;
            raw_copy.x += X_BED_SIZE / 2;
            float _z1 = fade_scaling_factor ? fade_scaling_factor * bilinear_z_offset(raw) : 0.0;
            float _z2 = fade_scaling_factor ? fade_scaling_factor * bilinear_z_offset(raw_copy) : 0.0;
            raw.z += max(_z1, _z2);
        }
        else
        {
