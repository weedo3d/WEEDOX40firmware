 ...
 */
// perron
int8_t SdBaseFile::readDir(dir_t* dir, char* longFilename, uint8_t* longlongFilename) {
  int16_t n;
    ...
    if (longFilename && DIR_IS_LONG_NAME(dir)) {
      ...
      if (VFAT->firstClusterLow == 0) {
        ...
        if (WITHIN(seq, 1, MAX_VFAT_ENTRIES)) {
          ...
          {
              // perron, save unicode char to longlonfFilename
              uint16_t _tc = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
              longFilename[n + i] = _tc & 0xFF;
