...
uint8_t GCodeQueue::priority_length = 0;
uint8_t GCodeQueue::priority_index_r = 0;
uint8_t GCodeQueue::priority_index_w = 0;
char GCodeQueue::priority_buffer[BUFSIZE][MAX_CMD_SIZE];
...
bool GCodeQueue::process_priority_command()
{
  if (priority_length == 0)
    return false;

  // copy priority command to command_buffer
  char cmd[MAX_CMD_SIZE];
  strcpy(cmd, priority_buffer[priority_index_r]);

  --priority_length;
  if (++priority_index_r >= BUFSIZE)
    priority_index_r = 0;
...
bool GCodeQueue::priority_enqueue_one(const char *cmd)
{
  if (*cmd == 0 || ISEOL(*cmd))
    return true;

  if (*cmd == ';' || priority_length >= BUFSIZE)
    return false;

  strcpy(priority_buffer[priority_index_w], cmd);

  if (++priority_index_w >= BUFSIZE)
    priority_index_w = 0;
  priority_length++;
...
bool GCodeQueue::priority_enqueue_one_P(PGM_P const pgcode)
{
  size_t i = 0;
  PGM_P p = pgcode;
  char c;
  ...
  memcpy_P(cmd, p, i);
  cmd[i] = '\0';
  return priority_enqueue_one(cmd);
}
...
inline void process_stream_char(const char c, uint8_t &sis, char (&buff)[MAX_CMD_SIZE], int &ind)
{
  ...
  {
    if (c == '"')
      sis = PS_NORMAL; // End quoted string
  }
...
// send "wait" to indicate Marlin is still waiting.
  ...
  // perron 200817, support wificam and esp32wifi
  while (length < BUFSIZE && MYSERIAL0.available())
  {
    ...
    if (ISEOL(serial_char))
    {
      ...
      if (process_line_done(serial_input_state[0], serial_line_buffer[0], serial_count[0]))
        continue;

      char *command = serial_line_buffer[0];
      ...
      if (npos)
      {
        ...
        if (gcode_N != last_N + 1 && !M110)
          return gcode_line_error(PSTR(STR_ERR_LINE_NO), 0);
        ...
        if (apos)
        {
          ...
          if (strtol(apos + 1, nullptr, 10) != checksum)
            return gcode_line_error(PSTR(STR_ERR_CHECKSUM_MISMATCH), 0);
        }
        else
          return gcode_line_error(PSTR(STR_ERR_NO_CHECKSUM), 0);
      ...
      if (IsStopped())
      {
        ...
        if (gpos)
        {
          ...
          case 0:
          case 1:
            PORT_REDIRECT(0); // Reply to the serial port that sent the command
      ...
      _enqueue(serial_line_buffer[0], true, 0);
    }
    else
      process_stream_char(serial_char, serial_input_state[0], serial_line_buffer[0], serial_count[0]);

  } // queue has space, serial has data

  while (length < BUFSIZE && MYSERIAL1.available())
  {
    const int c = read_serial(1);
    if (c < 0)
      break;
    ;
    ...
    {
      // Reset our state, continue if the line was empty
      if (process_line_done(serial_input_state[1], serial_line_buffer[1], serial_count[1]))
        continue;

      char *command = serial_line_buffer[1];
      ...
      if (M110)
      {
        char *n2pos = strchr(command + 4, 'N');
        if (n2pos)
          last_N = strtol(n2pos + 1, nullptr, 10);
        wt_echo_buffer_count();
        break;
      }
      ...
      if (npos)
      {
        ...
        if (gcode_N != last_N + 1)
          return gcode_line_error(PSTR(STR_ERR_LINE_NO), 1);
        ...
        if (apos)
        {
          ...
          while (count)
            checksum ^= command[--count];
          if (strtol(apos + 1, nullptr, 10) != checksum)
            return gcode_line_error(PSTR(STR_ERR_CHECKSUM_MISMATCH), 1);
        }
        else
          return gcode_line_error(PSTR(STR_ERR_NO_CHECKSUM), 1);
      ...
      if (IsStopped())
      {
        ...
        if (gpos)
        {
          ...
          case 5:
#endif
            PORT_REDIRECT(1); // Reply to the serial port that sent the command
            SERIAL_ECHOLNPGM(STR_ERR_STOPPED);
      ...
      // Add the command to the queue
      _enqueue(serial_line_buffer[1], true, 1);

      // perron, return buffer count to host
      wt_echo_buffer_count();
    }
    else
      process_stream_char(serial_char, serial_input_state[1], serial_line_buffer[1], serial_count[1]);
...
inline void GCodeQueue::get_sdcard_commands()
{
  ...
  while (length < BUFSIZE && !card_eof)
  {
    ...
    if (is_eol || card_eof)
    {
      ...
      if (!is_eol && sd_count)
        ++sd_count; // End of file with no newline
...
void GCodeQueue::get_available_commands()
{
  ...
  // {
  //   // copy priority command to command_buffer
  //   strcpy(command_buffer[index_w], priority_buffer[priority_index_r]);
