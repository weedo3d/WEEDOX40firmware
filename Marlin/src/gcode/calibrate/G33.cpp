...
static void print_calibration_settings(const bool end_stops, const bool tower_angles) {
  ...
  #ifdef DGUS_LCD
  dgus.ShowLogPair("Height: %.2f Radius: %.2f", delta_height, delta_radius);
  dgus.ShowLogPair("Endstop offset: X%.2f Y%.2f Z%.2f", delta_endstop_adj.a, delta_endstop_adj.b, delta_endstop_adj.c);
  dgus.ShowLogPair("Tower trim: X%.2f Y%.2f Z%.2f", delta_tower_angle_trim.a, delta_tower_angle_trim.b, delta_tower_angle_trim.c);
  #endif
}
...
void GcodeSuite::G33() {
  ...
  #ifdef DGUS_LCD
  dgus.ShowLog("G33 Auto Calibrate Begin...");
  #endif
  ...
  do { // start iterations
    ...
    if (verbose_level != 0) { // !dry run
      if ((zero_std_dev >= test_precision && iterations > force_iterations) || zero_std_dev <= calibration_precision) { // end iterations
        ...
        #ifdef DGUS_LCD
        dgus.ShowLog("Calibration Finish.");
        #endif
        SERIAL_ECHO_SP(32);
        #if HAS_BED_PROBE
        ...
        ui.set_status(mess);
        #ifdef DGUS_LCD
        dgus.ShowLog("Calibration setting:");
        #endif
        print_calibration_settings(_endstop_results, _angle_results);
        ...
        ui.set_status(mess);
        #ifdef DGUS_LCD
        dgus.ShowLogPair("Iteration: %02i std dev: %.2f", (unsigned int)iterations, zero_std_dev);
        #endif
        if (verbose_level > 1)
          print_calibration_settings(_endstop_results, _angle_results);
      }
