...
struct duration_t {
  ...
  // add by perron
  void toShortTimeString(char *buffer) const {
	  int y = this->year(),
		  d = this->day() % 365,
		  h = this->hour() % 24,
		  m = this->minute() % 60;
	  ...
	  else if (m) sprintf_P(buffer, PSTR("%im"), m);
	  else sprintf_P(buffer, PSTR("%im"), 0);
  }
  ...
  uint8_t toDigital(char *buffer, bool with_days=false) const {
    ...
    if (with_days) {
      uint16_t d = this->day();
      sprintf_P(buffer, PSTR("%hud%02huh%02hum"), d, h % 24, m);
      return d >= 10 ? 9 : 8;
    }
    else if (h < 100) {
      sprintf_P(buffer, PSTR("%02huh%02hum"), h, m);
      return 6;
    }
    else {
      sprintf_P(buffer, PSTR("%huh%02hum"), h, m);
      return 7;
    }
  }
};
